...
export const store = new Vuex.Store({
  ...
  actions: { // async
    ...
    runAutoOpeningGeneration (context) {
      ...
      let g = 0
      const maxEvalLossCp = Math.max(20, Number(cfg.autoGenerateMaxEvalLossCp) || 180)
      ...
      while (g < iterations) {
        ...
        for (let ply = 0; ply < maxPlies; ply++) {
          ...
          if (candidates.length) {
            ...
            const bestScore = Number(best && best.score) || 0
            const qualityCandidates = candidates.filter(c => {
              const cpGap = Math.max(0, bestScore - (Number(c.score) || 0))
              return cpGap <= maxEvalLossCp
            })
            ...
            if (qualityCandidates.length) {
              picked = chooseWeightedCandidate(qualityCandidates, {
                topK: inExploration ? (cfg.autoGenerateTopK || 3) : 1,
                temperature: inExploration ? (cfg.autoGenerateTemperature || 1) : 0.6
              })
            }
