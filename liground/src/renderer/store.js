      ...
      while (g < iterations) {
        ...
        for (let ply = 0; ply < maxPlies; ply++) {
          ...
          const analyzed = await context.dispatch('analyzeOpeningGenerationPosition', {
            fen: board.fen(),
            variant: context.state.variant,
            depth: Math.max(4, Number(cfg.autoGenerateDepth) || 12),
            topK: Math.max(2, cfg.autoGenerateTopK || 3)
          })
          console.log('[opening-gen] analysis result', { gameIndex: g + 1, ply: ply + 1, candidateCount: analyzed.length })
          const liveGraph = normalizeOpeningGraph(context.state.openingGraph || createOpeningGraph())
          const candidates = (analyzed && analyzed.length ? analyzed : openingCandidatesForFen(liveGraph, board.fen(), Math.max(2, cfg.autoGenerateTopK || 3)))
          ...
          try {
            ...
            console.log('[opening-gen] commit result', { gameIndex: g + 1, ply: ply + 1, move: picked.uci, committed })
            if (committed) {
      ...
      try {
        ...
        while (Date.now() - startedAt < 12000) {
          ...
          if ((bestmove && linesByPv.size) || (reachedDepth >= depth && linesByPv.size)) {
            await context.dispatch('stopEngine')
            console.log('[opening-gen] analyze resolve', { fen, reachedDepth, bestmove, lines: linesByPv.size })
        ...
        console.log('[opening-gen] analyze cleanup', { fen })
        await context.dispatch('stopEngine')
      }
      return []
    },
