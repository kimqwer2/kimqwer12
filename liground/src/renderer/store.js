...
export const store = new Vuex.Store({
  ...
  actions: { // async
    ...
    async runAutoOpeningGeneration (context) {
      context.commit('openingGeneration', { running: true, stopRequested: false, completedGames: 0, completedMoves: 0 })
      const cfg = context.state.openingBook || {}
      ...
      while (g < iterations) {
        ...
        for (let ply = 0; ply < maxPlies; ply++) {
          ...
          const candidates = (analyzed && analyzed.length ? analyzed : openingCandidatesForFen(graph, board.fen(), Math.max(2, cfg.autoGenerateTopK || 3)))
      ...
      context.commit('openingGraph', graph)
      context.dispatch('persistOpeningBook')
      context.commit('openingGeneration', { running: false, stopRequested: false })
